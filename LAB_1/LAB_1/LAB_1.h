...
Заголовочный файл проекта LAB_1
...
namespace LAB_1 {
	...
	private: unsigned short int n;							// Глобальная переменна, используемая для передачи в рекурсивную функцию значения из поля ввода, в области 1 и 2 рекурсии
	private: unsigned int n_dop;							// Глобальная переменна, используемая для передачи в рекурсивную функцию значения из поля ввода, в области дополнительной программы
	private: unsigned int q = 0;							// Глобальная переменна, используемая для фиксации количества рекурсивных вызовов
	private: unsigned short int i = 0;						// Глобальная переменная, используемая для инициализации массива int и передачи из него значений в рекурсивную функцию, в области дополнительной программы
	private: bool choice_check;								// Булевая переменная, отображающая выбранную рекурсию
	private: bool double_click_bool = false;				// Булевая переменная, ограничивающая повторный запуск рекурсии, в области дополнительной программы
	private: array<UInt32>^ arr = gcnew array<UInt32>(250);	// Массив int, используемый для фиксирования введенной последовательности, в области дополнительной программы
	//
...

	...
	// Принимает в качестве аргумента введенные в поле ввода данные (unsigned int)
	// Возвращает результат рекурсивных вычислений (int), функция от n
	//
	private: int recursion_1(unsigned int n)
	{
		++q;
		if (n == 1)
		{
	...
	// Возвращает результат рекурсивных вычислений (int), функция от n
	//
	private: int recursion_2(unsigned int n)
	{
		++q;
		if (n < 3)
		{
	...
	// Принимает в качестве аргументов два значения (int)
	// Возвращает максимальный из них (int)
	//
	private: unsigned int max(unsigned int a, unsigned int b)
	{
		if (a > b)
			return a;
		else
	...
	// Принимает числовую последовательность (int)
	// Возвращает максимальный элемент последовательности (int)
	//
	private: unsigned int dop()
	{
		q++;
		unsigned int num = 0;
		num = arr[i];
	...
	private: System::Void button2_Click(System::Object^ sender, System::EventArgs^ e) 
	{
		if (i > 255)
		{
			System::Windows::Forms::DialogResult result = MessageBox::Show("Введено максимальное количество элементов", "Ошибка", MessageBoxButtons::OK, MessageBoxIcon::Error);
			double_click_bool = true;
			...
			this->dop_output->Text = System::Convert::ToString(i);
		}
		else
		{
			if (!(this->textBox2->Text == ""))
			{
				arr[i] = System::Convert::ToInt32(delete_first_null(this->textBox2->Text));
				if (this->dop_input->Text == "")
					this->dop_input->Text = System::Convert::ToString(arr[i]);
				else
					this->dop_input->Text += "\r\n" + System::Convert::ToString(arr[i]);
				this->textBox2->Text = "";
				i++;
			}
			else
			{
				MessageBox::Show("Введите элемент последовательности.", "Ошибка", MessageBoxButtons::OK, MessageBoxIcon::Error);
			}
		}
	}
	//
	...
	private: System::Void dop_clear_btn_Click(System::Object^ sender, System::EventArgs^ e) 
	{
		...
		this->dop_input->Text = "";
		this->textBox2->Text = "";
		this->double_click_bool = false;
		this->q = 0;
		this->arr->Clear(arr, 0, arr->Length);
		this->call_quantity_output->Text = "";
	}
	//
	...
	{
		q = 0;
		if (this->double_click_bool == true)
		{
	...
	{
		String^ error_zero_first = "Первое элемент в последовательности не может быть нулём.";
		try
		{
			if (this->dop_input->Text == "" && this->textBox2->Text == "0")
			{
				throw error_zero_first;
			}
		}
		catch (String^ erroer_zero_first)
		{
			this->textBox2->ForeColor = Color::Red;
			System::Windows::Forms::DialogResult result = MessageBox::Show(error_zero_first, "Ошибка", MessageBoxButtons::OK, MessageBoxIcon::Error);
			if (result == System::Windows::Forms::DialogResult::OK)
			{
				this->textBox2->Text = "";
				this->textBox2->ForeColor = Color::Black;
	...
	{
		String^ error_not_a_number = "Введено неверное значение. Попробуйте ещё раз.";
		...
		try
		{
			for (int i = 0; i < this->textBox2->Text->Length; i++)
			{
				...
				{
					throw error_not_a_number;
				}
			}
		}
		catch (String^ error_not_a_number)
		{
			this->textBox2->ForeColor = Color::Red;
			System::Windows::Forms::DialogResult result = MessageBox::Show(error_not_a_number, "Ошибка", MessageBoxButtons::OK, MessageBoxIcon::Error);
			if (result == System::Windows::Forms::DialogResult::OK)
			{
				this->textBox2->Text = "";
				this->textBox2->ForeColor = Color::Black;
	...
	//
	//	Процедура, ограничивающая ввод числом 200.000.000, в области дополнительной программы
	//
	private: void dop_max_value_input()
	{
		unsigned int n_ = 200000000;
		String^ error_max = "Введено слишком большое число. Максимальное значение - 200.000.000.";
		if (this->textBox2->Text != "")
		{
			try
			{
				if (System::Convert::ToInt64(this->textBox2->Text) > n_)
				{
					throw error_max;
				}
			}
			catch (String^ error_max)
			{
				this->textBox2->ForeColor = Color::Red;
				System::Windows::Forms::DialogResult result = MessageBox::Show(error_max, "Ошибка", MessageBoxButtons::OK, MessageBoxIcon::Error);
				if (result == System::Windows::Forms::DialogResult::OK)
				{
					this->textBox2->Text = "";
					this->textBox2->ForeColor = Color::Black;
	...
	//
	//	Процедура, ограничивающая повторный ввод значений после предыдущих вычислений, в области дополнительной программы
	//
	private: void double_input_protect()
	{
		String^ double_input_protect = "Ответ уже вычислен. Нажмине кнопку \"Очистить поля\", чтобы ввести новую последовательность";
		try
		{
			if (!(this->dop_output->Text == "") && !(this->textBox2->Text == ""))
				throw double_input_protect;
		}
		catch (String^ double_input_protect)
		{
			this->textBox2->ForeColor = Color::Red;
			System::Windows::Forms::DialogResult result = MessageBox::Show(double_input_protect, "Ошибка", MessageBoxButtons::OK, MessageBoxIcon::Error);
			if (result == System::Windows::Forms::DialogResult::OK)
			{
				this->textBox2->Text = "";
				this->textBox2->ForeColor = Color::Black;
	...
	//
	//	Функция, удаляющая первые незначащие нули числа, в области дополнительной программы
	//
	private: String^ delete_first_null(String^ data)
	{
		int count = 0;
		bool check = true;
	...
	//
	// Тест дополнительной рекурсивной функции по определению максимального элемента последовательности, ожидаемый результат 4
	//
	private: int test_6()
	{
		for (i = 0; i < 5; i++)
	...
	//
	// Процедура запускающая тестирование четырёх функций
	//
	private: void test_cases()
	{
		int test_flag = (test_1() + test_2() + test_3() + test_4() + test_5() + test_6());
		if (test_flag == 0)
		{
	...
	//
	// Процедура, передаваемая в конструктор формы, запускающая тестирование четырёх функций
	//
